...
JSONEditor.prototype = {
    ...
    getFormValue: function() {
        ...
        for (var key in cloneOfValue) {
            if (key.indexOf("$group_") == 0) {
                var subObj = cloneOfValue[key];
                for (var subKey in subObj) {
                    cloneOfValue[subKey] = subObj[subKey];
                }
                delete cloneOfValue[key]; //删除原始key
            }
        }
        return cloneOfValue;
    },
    ...
    setFormValue: function(value){
        ...
        if(this.schema&&this.schema.properties){
            ...
            for(var key in props){
                if(key.indexOf("$group_") == 0){
                    value[key] = {};
                    var groupProps = props[key].properties;
                    for(var groupPropKey in groupProps){
                        if(value[groupPropKey])value[key][groupPropKey]=value[groupPropKey];
                        delete value[groupPropKey];
                    }
                }
            }
        }
    ...
    getPropTitle: function(path){
        ...
        var propTitle = "";
        var schemaProp = this.schema.properties;
        for(var prop in schemaProp){
            if(prop === pathPropArray[1]){
                if(prop.indexOf("$group_")===-1){
                    propTitle = schemaProp[prop].title;
                }else{
                    for(var groupProp in schemaProp[prop].properties){
                        if(groupProp === pathPropArray[2]){
                            propTitle = schemaProp[prop].properties[groupProp].title;
                        }
                    }
                }
            }
...
JSONEditor.defaults.editors.object = JSONEditor.AbstractEditor.extend({
  ...
  build: function() {
    ...
    // If the object should be rendered as a div
      ...
      if(this.getTitleClass()){
        this.header.className = this.getTitleClass();
      }
